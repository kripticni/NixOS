...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) {
    /* status is only drawn on selected monitor */ // comment out to draw on
                                                   // both monitors its jank tho
    //	char *text, *s, ch;
...
void focus(Client *c) {
  ...
  if (c) {
     ...
     * is restored */
    //		if (!(solitary(c) - 1)) { krakens weird patch
    if (!solitary(c)) {
...
void quit(const Arg *arg) {
  system("if pgrep '^polybar' > /dev/null; then pkill --signal sigterm "
         "polybar; fi");
